...
struct ContentView: View {
    ...
    @State private var editingEntry: FirewoodTallyEntry?
...
struct EntryEditView: View {
    ...
    let entry: FirewoodTallyEntry?
    let onSave: (FirewoodTallyEntry) -> Void
    ...
    init(entry: FirewoodTallyEntry?, onSave: @escaping (FirewoodTallyEntry) -> Void) {
        self.entry = entry
        self.onSave = onSave
        _title = State(initialValue: entry?.title ?? "")
        _date = State(initialValue: entry?.date ?? Date())
        _f1 = State(initialValue: entry?.cords ?? "")
        _f2 = State(initialValue: entry?.species ?? "")
